		...
		<tbody>
						...
						<tr>
							...
							<td class="text-center">
								<a data-placement="left" data-popup="tooltip" title="Print Project Scope" style="margin: 10px; color: green;" href="<?=base_url()?>report/printScope/<?=$key->projectScopeID?>"><i class="icon-printer"></i></a>
	                            ...
	                            <?php }elseif ($stats == 'rejected'){?>
	                            	<button data-placement="left" data-popup="tooltip" title="You Was Rejected" style="<?=showLevel(array(55))?> margin: 10px">Rejected</button>
	                            <?php }else{
	                            if ($check2 == false) {?>
	                            <a data-placement="left" data-popup="tooltip" title="Apply This Project Scope" onclick="confirms('Apply','Apply Project Scope `<?=$key->projectScope?>` ?','<?=base_url('scope/applyScope')?>','<?=$key->projectScopeID?>')" style="<?=showLevel(array(55))?> margin: 10px; color: white;" class="btn btn-primary btn-sm <?=$button?>"><i class="icon-checkmark"></i> Apply</a><br/><?=$badge?>
	                        	<?php }else{?>
	                            '<a data-placement="left" data-popup="tooltip" title="Cancel This Project Scope" onclick="confirms('Cancel','Cancel Project Scope `<?=$key->projectScope?>` ?','<?=base_url('scope/cancelScope')?>','<?=$key->projectScopeID?>')" style="<?=showLevel(array(55))?> margin: 10px; color: white;" class="btn btn-danger btn-sm"><i class="icon-cross2"></i> Cancel</a>,
	                            <?php } } }?>							
							</td>
						</tr>
						<?php
