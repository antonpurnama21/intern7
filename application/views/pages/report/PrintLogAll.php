...
class Coba extends Fpdf {
	...
	$pdf->Cell(100 ,15, "Log Time" ,1 ,"LR", "C", true);
	$pdf->Cell(100 ,15, "Browser" ,1 ,"LR", "C", true);
	$pdf->Cell(100 ,15, "IP Address" ,1 ,"LR", "C", true);
	$pdf->Cell(100 ,15, "Platform" ,1 ,"LR", "C", true);
	$pdf->Cell(100, 15, "Type Log", 1, "LR", "C", true);
	$pdf->Cell(170 ,15, "Description" ,1 ,"LR", "C", true);
	if (!empty($data)) {
		$pdf->SetLeftMargin(20);
		$pdf->Ln();
		$no = 0;
		...
		foreach ($data as $key) {
			...
			$pdf->MultiCell(100,15,$key->logIP,'LRT', 'C');
			$pdf->SetXY($pdf->GetX()+420, $curY);
			$pdf->MultiCell(100,15,$key->logPlatform,'LRT', 'C');
